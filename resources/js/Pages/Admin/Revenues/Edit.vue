        ...
        <div class="mx-auto max-w-5xl">
          ...
          <div class="mb-8">
            <div class="flex items-center justify-between">
              <div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Revenue</h1>
                <p class="mt-2 text-lg text-gray-600">Update revenue transaction record</p>
              </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div class="relative">
                      <input
                        type="text"
                        :value="revenue.receipt_number"
                        disabled
                        class="w-full px-4 py-3 bg-gradient-to-r from-gray-100 to-gray-50 border-2 border-gray-300 rounded-xl cursor-not-allowed text-gray-600 font-semibold"
                      >
                      <div class="absolute right-3 top-1/2 transform -translate-y-1/2">
                        <span class="px-3 py-1 bg-gray-500 text-white text-xs font-bold rounded-lg shadow-md">READ-ONLY</span>
                      </div>
                    </div>
            ...
            <div class="bg-white shadow-lg rounded-2xl border border-gray-100 p-6">
              <div class="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
                ...
                <div class="flex space-x-3">
                  ...
                  <button
                    type="submit"
                    class="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl text-white font-bold hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 flex items-center space-x-2 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105"
                  ...
                  >
                    ...
                    <span>{{ form.processing ? 'Updating...' : 'Update Revenue' }}</span>
                  </button>
                </div>
              </div>
            </div>
...
const form = useForm({
  counselor_id: props.revenue.counselor_id,
  marketer_id: props.revenue.marketer_id || '',
  lead_id: props.revenue.lead_id || '',
  date: props.revenue.date,
  amount: props.revenue.amount,
  payment_mode: props.revenue.payment_mode,
  transaction_id: props.revenue.transaction_id || '',
  receipt_number: props.revenue.receipt_number,
  course_type: props.revenue.course_type || '',
  payment_type: props.revenue.payment_type,
  installment_number: props.revenue.installment_number || '',
  total_installments: props.revenue.total_installments || '',
  discount_applied: props.revenue.discount_applied || 0,
...
const filterLeadsByCounselor = () => {
  ...
  // Don't reset lead_id if it's already valid for the selected counselor
  if (form.lead_id) {
    const leadExists = filteredLeads.value.find(lead => lead.id == form.lead_id);
    if (!leadExists) {
      form.lead_id = '';
    }
  }
...
const submit = () => {
  form.put(route('admin.revenues.update', props.revenue.id), {
    preserveScroll: true,
  });
};
